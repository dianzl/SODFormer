...
class Multimodal_detr_asyn(nn.Module):
    ...
    def update_aps(self, aps, aps_id, video_id):
        self.aps_id = aps_id
        aps_src, mask_flatten, spatial_shapes, valid_ratios = self.aps_tem_tran(aps, video_id)
        self.aps_cur_dict['aps_src'] = aps_src
        self.aps_cur_dict['mask_flatten'] = mask_flatten
        self.aps_cur_dict['spatial_shapes'] = spatial_shapes
        self.aps_cur_dict['valid_ratios'] = valid_ratios
        return

    def forward(self, aps, dvs, video_id, frame_id):
        ...
        if frame_id > self.aps_id:
            self.update_aps(aps, frame_id, video_id)

        aps_src, mask_flatten, spatial_shapes, valid_ratios = self.aps_cur_dict.values()
...
def build_Multimodal_ST(args):
    ...
    Multimodal_ST_model = Multimodal_detr_asyn(train_temporal=train_temporal,
